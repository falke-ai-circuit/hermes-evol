...
PROVIDER_ENDPOINTS: Dict[str, dict] = {
    ...
    "minimax": {
        ...
        "description": "MiniMax M2.7 — fast, cheap, OpenAI-compatible. Good for EVOL scoring/reflection.",
    },
}
...
class EvolConfig:
    ...
    # ── Global ──
    mode: Literal["profile", "global", "per_agent"] = "profile"
    enabled: bool = True
    ...
    # Verbosity: 0=silent, 1=summary (default), 2=detailed, 3=full debug
    verbosity: int = 1
    # Dry-run: if True, run ALL phases but write NOTHING to files/tier2
    dry_run: bool = False
    ...
    # ── Notification level (controls which post-phase hooks fire) ──
    # "off"   → no hooks fire, no notifications, no video
    # "low"   → notification hooks fire (reflect, explore, memorize), express video skipped (DEFAULT)
    # "high"  → all hooks fire including express video generation
    notify_level: Literal["off", "low", "high"] = "low"
    ...
    def __init__(self, profile: Optional[str] = None, mode: Optional[str] = None):
        ...
        self.verbosity = 1   # 0=silent, 1=summary, 2=detailed, 3=full
        self.dry_run = False
        self.approval_mode = "suggested"  # "auto"/"yolo" = autonomous, "suggested" = needs Goran approval
        self.tier2_backend = "graphiti"   # graphiti | mem0 | ladybug | dummy
        self.tier2_config = {}            # backend-specific config
        self.notify_level = "low"  # off | low | high
        self.phase_enabled = {
        ...
        self.phase_post_commands: Dict[str, str] = {}
    ...
    def _load_from_file(self):
        ...
        if "approval_mode" in data:
            self.approval_mode = data["approval_mode"]
        if "tier2_backend" in data:
            self.tier2_backend = data["tier2_backend"]
        if "tier2_config" in data:
            self.tier2_config = data["tier2_config"]
        ...
        if "per_agent_min_completions" in data:
            self.per_agent_min_completions = data["per_agent_min_completions"]
        if "per_agent_cooldown_hours" in data:
            self.per_agent_cooldown_hours = data["per_agent_cooldown_hours"]
        if "per_agent_global_fallback_hours" in data:
            self.per_agent_global_fallback_hours = data["per_agent_global_fallback_hours"]
        ...
        # Notification level
        if "notify_level" in data:
            self.notify_level = data["notify_level"]
    ...
    def _apply_env_overrides(self):
        ...
        env_map = {
            ...
            "EVOL_VERBOSITY": "verbosity",
            "EVOL_DRY_RUN": "dry_run",
        ...
        for env_var, attr in env_map.items():
            ...
            if attr == "enabled":
                ...
                if val in ("profile", "global", "per_agent"):
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "per_agent_min_completions": self.per_agent_min_completions,
            "per_agent_cooldown_hours": self.per_agent_cooldown_hours,
            "per_agent_global_fallback_hours": self.per_agent_global_fallback_hours,
